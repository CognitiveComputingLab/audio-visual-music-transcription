...
class HandFinder:
	...
	SKIN_LOWER = np.array([0, 40, 70], dtype='uint8')
	SKIN_UPPER = np.array([100, 255, 255], dtype='uint8')
	...
	# def mediapipe_skeletal(self, frame):
	# 	try:
	# 		rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
	# 	except cv2.error as e:
	# 		print(f"Failed to convert frame color format: {e}")
	# 		return None, None
	# 	# Find landmarks
	# 	print("Finding LANDMARKS FOUND")    
	# 	results = self.hands.process(rgb_frame)
	# 	print("still finding landmarks")
	...
	# 			print("INSIDE LOOP")
	# 			try:
	# 				# Ensure that hand_landmarks is a single LandmarkList object
	# 				if hasattr(hand_landmarks, 'landmark'):
	# 					# Create a hand mask
	# 					hand_mask = self.draw_hand_skeleton(frame, hand_landmarks)
	# 					# Optionally, draw the landmarks on the original frame for visualization
	...
	# 				print(f"Error in draw_hand_skeleton: {e}")
	# 		return None, None

	# 	return frame, hand_mask

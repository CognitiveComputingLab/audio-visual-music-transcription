import cv2
import numpy as np

from piano_vision.helpers import apply_mask, centre_of_contour
from piano_vision.processors import KeysManager, HandFinder


class PressedKeyDetector:
	MIN_CONTOUR_AREA = 100
	STICKINESS = 2

	def __init__(self, ref_frame, keys_manager): #hand_finder
		self.ref_frame = ref_frame
		self.keys_manager: KeysManager = keys_manager
		# self.hand_finder: HandFinder = hand_finder 
		self.currently_pressed = set()
		self.to_be_added = dict()
		self.to_be_removed = dict()

# No need for key press detection as passing in candidates of general finger location will be produce more accurate results
# With key presses
	def detect_pressed_keys(self, frame, skin_mask, fingertips=None):
		frame = frame.copy()
		# Dilate again to ensure that we don't include any small bits of skin
		kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
		dilated_mask = cv2.dilate(skin_mask, kernel, iterations=1)

		skin = apply_mask(frame, dilated_mask)
		frame = cv2.subtract(frame, skin)
		# cv2.imshow('skin_masked_out', frame)

		skin_ref = apply_mask(self.ref_frame, dilated_mask)
		ref = cv2.subtract(self.ref_frame, skin_ref)

		diff = self.get_diff(frame, ref)

		contours, hierarchy = cv2.findContours(diff, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
		contours = tuple(filter(lambda c: cv2.contourArea(c) > self.MIN_CONTOUR_AREA, contours))
		cv2.drawContours(frame, contours, -1, color=(0, 255, 0), thickness=cv2.FILLED)

		centres = tuple(map(centre_of_contour, contours))

		for centre in centres:
			cv2.circle(frame, (centre[0], centre[1]), radius=5, color=(0, 0, 255), thickness=cv2.FILLED)
		cv2.imshow('frame_with_diff', frame)

		pressed_keys = set()
		for centre in centres:
			for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]:
				if key.x < centre[0] < key.x + key.width and key.y < centre[1] < key.y + key.height:
					pressed_keys.add(key)

		if fingertips:
			pressed_keys = tuple(filter(
				# Filter pressed keys to only those which contain a fingertip
				lambda k: any(map(lambda f: self.fingertip_within_key(f, k), fingertips)),
				pressed_keys
			))

		self.process_sticky_pressed_changes(pressed_keys)
		return self.currently_pressed

# # Candidate keys based on fingertip positions
# 	def detect_pressed_keys(self, frame, skin_mask, fingertips=None):
# 		frame = frame.copy()
# 		kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
# 		dilated_mask = cv2.dilate(skin_mask, kernel, iterations=1)
# 		skin = apply_mask(frame, dilated_mask)
# 		frame = cv2.subtract(frame, skin)
# 		skin_ref = apply_mask(self.ref_frame, dilated_mask)
# 		ref = cv2.subtract(self.ref_frame, skin_ref)
# 		diff = self.get_diff(frame, ref)

# 		contours, hierarchy = cv2.findContours(diff, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
# 		contours = tuple(filter(lambda c: cv2.contourArea(c) > self.MIN_CONTOUR_AREA, contours))

# 		# Detect keys isntead based on fingertip positions
# 		if fingertips:
# 			pressed_keys = set()
# 			for fingertip in fingertips:
# 				for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]:
# 					if self.fingertip_within_key(fingertip, key):
# 						pressed_keys.add(key)
# 			self.process_sticky_pressed_changes(pressed_keys)
# 		else:
# 			pressed_keys = set([key for centre in tuple(map(centre_of_contour, contours))
# 								for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]
# 								if key.x < centre[0] < key.x + key.width and key.y < centre[1] < key.y + key.height])
# 			self.process_sticky_pressed_changes(pressed_keys)

# 		return self.currently_pressed

	# def draw_hand_skeleton(self, frame, landmarks):
	# 	print("processing")
	# 	mask = np.zeros_like(frame, dtype=np.uint8)
	# 	if landmarks is not None:
	# 		self.mp_drawing.draw_landmarks(
  #       mask,
  #       landmarks,
  #       self.mp_hands.HAND_CONNECTIONS,
  #       self.mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=2, circle_radius=2),
  #       self.mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=2))
	# 	else:
	# 			print("returning mask")
	# 	return mask
	
	# def detect_pressed_keys(self, frame, hand_landmarks):
	# 	rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
	# 	print("done")
	# 	frame = frame.copy()
	# 	mask = self.draw_hand_skeleton(frame, hand_landmarks)
	# 	print("done this too")
	# 	# Dilate the mask to improve detection robustness
	# 	kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
	# 	dilated_mask = cv2.dilate(mask, kernel, iterations=1)
	# 	print("dilated")
	# 	# Find contours which could be overlapping with the keys
	# 	version = cv2.__version__.split('.')[0]
	# 	_, contours, hierarchy = cv2.findContours(dilated_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	# 	pressed_keys = set()
	# 	print("done")
	# 	# Loop through contours and check for intersection with keys
	# 	for contour in contours:
	# 		contour_area = cv2.contourArea(contour)
	# 		if contour_area > self.MIN_CONTOUR_AREA:
	# 			x, y, w, h = cv2.boundingRect(contour)
	# 			for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]:
	# 				if key.x < x + w and key.x + key.width > x and key.y < y + h and key.y + key.height > y:
	# 					pressed_keys.add(key)
	# 	print("done")
	# 	self.process_sticky_pressed_changes(pressed_keys)
	# 	return self.currently_pressed

	# def draw_hand_skeleton(self, frame, landmarks):
	# 	print("Processing skeleton drawing...")
	# 	mask = np.zeros_like(frame, dtype=np.uint8)
	# 	try:
	# 		if landmarks:
	# 			print("landmarks exist")
	# 			# Enhance landmarks
	# 			for landmark in landmarks.landmark:
	# 				print("loop entered")
	# 				x = int(landmark.x * frame.shape[1])
	# 				y = int(landmark.y * frame.shape[0])
	# 				cv2.circle(mask, (x, y), 10, (255, 255, 255), -1)  # Use a visible radius
	# 			print("Landmarks drawn on mask.")
	# 		else:
	# 			print("No landmarks to draw.")
	# 	except Exception as e:
	# 			print(f"Error inside draw_hand_skeleton: {e}")
	# 	return mask

	# def detect_pressed_keys_landmark(self, frame, hand_landmarks):
	# 	print("Detecting pressed keys...")
	# 	mask = self.hand_finder.draw_hand_skeleton(frame, hand_landmarks)
	# 	print("mask created")
	# 	# Check each key for overlap with landmark points
	# 	pressed_keys = set()
	# 	if hand_landmarks:
	# 		for landmark in hand_landmarks.landmark:
	# 			x = int(landmark.x * frame.shape[1])
	# 			y = int(landmark.y * frame.shape[0])
	# 			for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]:
	# 				if key.x <= x <= key.x + key.width and key.y <= y <= key.y + key.height:
	# 					pressed_keys.add(key)
	# 					print(f"Key {key} pressed by landmark at ({x}, {y}).")

	# 	self.process_sticky_pressed_changes(pressed_keys)
	# 	return self.currently_pressed

	def detect_pressed_keys(self, frame, skin_mask, fingertips=None):
		frame = frame.copy()
		kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
		dilated_mask = cv2.dilate(skin_mask, kernel, iterations=1)
		skin = apply_mask(frame, dilated_mask)
		frame = cv2.subtract(frame, skin)
		skin_ref = apply_mask(self.ref_frame, dilated_mask)
		ref = cv2.subtract(self.ref_frame, skin_ref)
		diff = self.get_diff(frame, ref)

		contours, hierarchy = cv2.findContours(diff, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
		contours = tuple(filter(lambda c: cv2.contourArea(c) > self.MIN_CONTOUR_AREA, contours))

		# Detect keys isntead based on fingertips' positions
		if fingertips:
			pressed_keys = set()
			for fingertip in fingertips:
				for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]:
					if self.fingertip_within_key(fingertip, key):
						pressed_keys.add(key)
			self.process_sticky_pressed_changes(pressed_keys)
		else:
			# Use the original method if this does not work correctly
			pressed_keys = set([key for centre in tuple(map(centre_of_contour, contours))
								for key in [*self.keys_manager.white_keys, *self.keys_manager.black_keys]
								if key.x < centre[0] < key.x + key.width and key.y < centre[1] < key.y + key.height])
			self.process_sticky_pressed_changes(pressed_keys)

		return self.currently_pressed

	def process_sticky_pressed_changes(self, pressed_keys):
		# Key identified but is no longer pressed, remove from to_be_added
		delete_after = []
		for key in self.to_be_added:
			if key not in pressed_keys:
				delete_after.append(key)
		for key in delete_after:
			del self.to_be_added[key]

		for key in pressed_keys:
			# Key removed but is pressed again, remove from to_be_removed
			if key in self.to_be_removed:
				del self.to_be_removed[key]
			if key not in self.currently_pressed:
				# Key added and and is still pressed, reduce counter by 1
				if key in self.to_be_added:
					self.to_be_added[key] -= 1
					# Key is to be added
					if self.to_be_added[key] == 0:
						del self.to_be_added[key]
						self.currently_pressed.add(key)
				# Otherwise add it in future if not seen before
				else:
					self.to_be_added[key] = self.STICKINESS

		delete_after = []
		for key in self.currently_pressed:
			if key not in pressed_keys:
				if key in self.to_be_removed:
					self.to_be_removed[key] -= 1
					if self.to_be_removed[key] == 0:
						del self.to_be_removed[key]
						delete_after.append(key)
				else:
					self.to_be_removed[key] = self.STICKINESS
		for key in delete_after:
			self.currently_pressed.remove(key)

	@staticmethod
	def fingertip_within_key(fingertip, key):
		return key.x < fingertip[0] < (key.x + key.width) and key.y < fingertip[1] < (key.y + key.height)

	@staticmethod
	def get_diff(frame, ref):
		diff = cv2.absdiff(frame, ref)
		diff = cv2.cvtColor(diff, cv2.COLOR_BGR2GRAY)
		# diff = cv2.GaussianBlur(diff, (3, 3), 0)
		diff = cv2.adaptiveThreshold(diff, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 11, 10)
		# cv2.imshow('binary_diff', diff)
		kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
		diff = cv2.dilate(diff, kernel, iterations=2)
		diff = cv2.erode(diff, kernel, iterations=2)
		kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10, 10))
		diff = cv2.morphologyEx(diff, cv2.MORPH_OPEN, kernel, iterations=1)
		diff = cv2.morphologyEx(diff, cv2.MORPH_CLOSE, kernel, iterations=1)
		# cv2.imshow('morphed_diff', diff)
		return diff

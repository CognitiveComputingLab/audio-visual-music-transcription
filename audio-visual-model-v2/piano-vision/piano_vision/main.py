...
class PianoVision:
	...
	def main_loop(self):
		...
		with VideoReader(self.video_file) as video_reader:
			...
			handled, rotation_required, mask = self.handle_reference_frame(initial_frame)
			# print("handled")
			...
			# # Pre=process the first frame for segmentation
			# processed_frame = self.pre_process_frame(initial_frame)
			# # Apply the segmentation to the first frame
			# initial_frame, mask = self.bounder.keyboard_segmentation(processed_frame, initial_frame)

			# # Ensure masked_image is properly formatted for display
			# if initial_frame.dtype != np.uint8:
			# 	initial_frame = np.clip(initial_frame * 255, 0, 255).astype(np.uint8)

			# Now use masked_image for imshow
			# cv2.imshow("INITIAL", initial_frame)
			# cv2.waitKey(0)
			...
			while frame is not None:
				...
				# Applies the initial segmentation mask
				frame = self.bounder.apply_segmentation_mask(mask, frame)
				...
				# print("segmentation and rotate done")

				# # Detect the hands
				# processed_frame, skeletal_mask = self.hand_finder.mediapipe_skeletal(keyboard)
				# print("skeletal detected")

				# # Assuming 'processed_frame' is the frame with drawn landmarks
				# plt.figure(figsize=(10, 10))  # Set the figure size as needed
				# plt.imshow(cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB))  # Convert BGR to RGB for correct color display
				# plt.axis('off')
				# plt.show()

				# Display the processed frame
				# cv2.imshow('Frame', processed_frame)
				# if skeletal_mask is not None:
				# 	cv2.imshow('Hand Mask', skeletal_mask)
				...
				# pressed_keys = self.pressed_key_detector.detect_pressed_keys_landmarks(keyboard, skeletal_mask)
	...
	def pre_process_frame(self, frame):
		...
		# # Transpose the image dimensions from HWC to CHW
		# frame_transposed = np.transpose(frame_resized, (2, 0, 1))
		# # Convert to a torch tensor
		# frame_tensor = torch.tensor(frame_transposed, dtype=torch.float32)
		...
		# frame_tensor = Image.open(frame)
		# image = transform(image).unsqueeze(0)

		# # Ensure frame is in RGB format if coming from OpenCV
	...
	def handle_reference_frame(self, reference_frame):
		...
		rotation, rotation_required = self.bounder.find_rotation(reference_frame)
		# print('rotation: {}'.format(rotation))
		...
		# Pre=process the first frame for segmentation
		processed_frame = self.pre_process_frame(reference_frame)
		# print("done seg preprocess")
		# Apply the segmentation to the first frame
		reference_frame, mask = self.bounder.keyboard_segmentation(processed_frame, reference_frame)
		# print("done seg")
	...
	# 			print(best_candidate, best_brightness, best_black_key_count)
		...
		return True, rotation_required, mask

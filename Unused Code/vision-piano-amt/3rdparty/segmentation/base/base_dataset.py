...
class BaseDataSet(Dataset):
    ...
    def _set_files(self):
        ...
        self.files = [
            {
                "img": os.path.join(image_dir, file),
                "mask": os.path.join(mask_dir, file.replace(".jpg", ".png"))
            }
            for file in os.listdir(image_dir) if file.endswith(".jpg")
        ]
        print(f"Loaded {len(self.files)} files from {image_dir}")
        if len(self.files) > 0:
            print(f"Sample file: {self.files[0]}")
        else:
            print("No files found, check the directory path and file extensions.")
        # raise NotImplementedError
    
    def _load_data(self, index):
        img_path = self.files[index]["img"]

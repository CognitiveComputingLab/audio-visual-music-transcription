...
class KeysManager:
	...
	def threshold(self, frame):
		...
		# Dynamically adjust the block size for adaptive thresholding
		# block_size = int(min(frame.shape[:2]) * 0.05) | 1  # Block size must be odd.
		# C = 10  # This is a parameter you may need to tune.
	...
	def find_key_contours(self, thresholded):
		#Check keys in the top two quarters of the image
		...
		# Filter the contours appropriately
	...
	def label_keys(self):
		...
		# Only keep the black and white keys we are certain of
		self.black_keys = [key for key in self.black_keys if key.note is not None]
		self.white_keys = [key for key in self.white_keys if key.note is not None]

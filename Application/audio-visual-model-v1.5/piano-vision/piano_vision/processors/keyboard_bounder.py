...
class KeyboardBounder:
    ...
    def bound_transform_and_flip(self, frame, bounds, true_bounds):
        ...
        width = int(x_plus_w - x)
        height = int(y_plus_h - y)
    ...
    def bound_transform_and_flip_2(self, frame, true_bounds):
        def order_corners(points):
        ...
        true_bounds = order_corners(true_bounds)
        true_bounds = np.array(true_bounds, dtype=np.float32)
    
        # Find the minimum and maximum x and y coordinates
        min_x = np.min(true_bounds[:, 0])
        max_x = np.max(true_bounds[:, 0])
        min_y = np.min(true_bounds[:, 1])
        max_y = np.max(true_bounds[:, 1])

        # Create the bounds in the format [(top-left), (top-right), (bottom-right), (bottom-left)]
        bounds = np.array([
            [min_x, min_y],
            [max_x, min_y],
            [max_x, max_y],
        ...
        # Check which half has more black pixels and possibly flip the image
        top_half = warped[:height // 2, :]
        bottom_half = warped[height // 2:, :]
        black_thresh = 50
        top_black = np.sum(np.all(top_half < black_thresh, axis=2))
        ...
        if bottom_black > top_black:
            warped = cv2.flip(warped, 0)  # Vertical flip
